...
pub fn validate_missing_test_files(
    ...
    let test_coverage_for =
        |relative: &std::path::Path,
         test_files: &std::collections::HashSet<String>,
         test_dirs: &std::collections::HashSet<String>| {
            let file_name = relative.file_stem().and_then(|s| s.to_str()).unwrap_or("");
            if file_test_keys(file_name)
                .iter()
                .any(|k| test_files.contains(k))
            {
                return true;
            }
    ...
    for crate_dir in validator.get_crate_dirs()? {
        ...
        for_each_file_under_root(
            ...
            |entry| {
                ...
                if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
                    test_files.extend(file_test_keys(stem));
        ...
        for_each_file_under_root(
            ...
            |entry| {
                ...
                if !test_coverage_for(relative, &test_files, &test_dirs) {
