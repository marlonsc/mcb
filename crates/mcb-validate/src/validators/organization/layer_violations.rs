...
pub fn validate_layer_violations(config: &ValidationConfig) -> Result<Vec<OrganizationViolation>> {
    ...
    for_each_scan_file(config, Some(LanguageId::Rust), true, |entry, _src_dir| {
        ...
        scan_layer_file(
            ...
            path_str,
            &content,
            (&arc_new_service_pattern, &server_import_pattern),
            &mut violations,
        );
        Ok(())
    })?;
...
/// Scan one file for layer-boundary violations: server-layer service creation
/// and application/infrastructure imports from the server layer.
fn scan_layer_file(
    path: &Path,
    path_str: &str,
    content: &str,
    ...
    let (arc_new_service_pattern, server_import_pattern) = patterns;
