...
use mcb_domain::ports::repositories::project_repository::IssueFilter;

pub struct MockProjectRepository {
    projects: Arc<Mutex<HashMap<String, Project>>>,
    phases: Arc<Mutex<HashMap<String, ProjectPhase>>>,
    issues: Arc<Mutex<HashMap<String, ProjectIssue>>>,
    dependencies: Arc<Mutex<HashMap<String, ProjectDependency>>>,
    decisions: Arc<Mutex<HashMap<String, ProjectDecision>>>,
}
...
impl ProjectRepository for MockProjectRepository {
    ...
    async fn create_phase(&self, phase: &ProjectPhase) -> Result<()> {
        self.phases
            .lock()
            .expect("Lock poisoned")
            .insert(phase.id.clone(), phase.clone());
        Ok(())
    }

    async fn get_phase(&self, id: &str) -> Result<Option<ProjectPhase>> {
        let phases = self.phases.lock().expect("Lock poisoned");
    ...
    async fn filter_issues(&self, filter: &IssueFilter) -> Result<Vec<ProjectIssue>> {
        let issues = self.issues.lock().expect("Lock poisoned");
        let mut results: Vec<ProjectIssue> = issues
            .values()
            .filter(|i| {
                ...
                if let Some(ref phase_id) = filter.phase_id
                    && i.phase_id.as_ref() != Some(phase_id)
                {
                    return false;
                }
                ...
                if let Some(ref assignee) = filter.assignee
                    && i.assignee.as_ref() != Some(assignee)
                {
                    return false;
                }
                if let Some(ref label) = filter.label
                    && !i.labels.contains(label)
                {
                    return false;
                }
                true
    ...
    async fn add_dependency(&self, dep: &ProjectDependency) -> Result<()> {
        self.dependencies
            .lock()
            .expect("Lock poisoned")
            .insert(dep.id.clone(), dep.clone());
        Ok(())
    }

    async fn remove_dependency(&self, id: &str) -> Result<()> {
        self.dependencies.lock().expect("Lock poisoned").remove(id);
        Ok(())
    }

    async fn list_dependencies(&self, _project_id: &str) -> Result<Vec<ProjectDependency>> {
        let dependencies = self.dependencies.lock().expect("Lock poisoned");
        Ok(dependencies.values().cloned().collect())
    }
