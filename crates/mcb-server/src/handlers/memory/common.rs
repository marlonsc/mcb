...
use crate::utils::mcp::{OriginPayloadFields, resolve_org_id, resolve_origin_context};
...
pub(super) fn resolve_memory_origin_context(
    ...
    origin_context.parent_session_id_correlation = parent_session_hash;
    ...
    F: Fn(&MemorySearchResult) -> Option<Value>,
{
    ...
    match memory_service
        .search_memories(&org_id, spec.query, Some(filter), fetch_limit)
        .await
    {
        Ok(results) => {
