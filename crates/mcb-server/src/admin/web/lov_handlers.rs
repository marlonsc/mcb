...
mod tests {
    ...
    #[test]
    fn test_display_field_organizations() {
        let entity = AdminRegistry::find("organizations").unwrap();
        let field = display_field(entity);
        assert_eq!(
            field, "name",
            "Organizations should use 'name' as display field"
        );
    }

    #[test]
    fn test_display_field_plans() {
        let entity = AdminRegistry::find("plans").unwrap();
        let field = display_field(entity);
        // Plans should pick a text field like "name" or "title"
        assert_ne!(
            field, "id",
            "Plans should have a display field other than id"
        );
    }

    #[test]
    fn test_display_field_fallback() {
        // issue-label-assignments has mostly FK/id fields, may fall back to "id"
        let entity = AdminRegistry::find("issue-label-assignments").unwrap();
        let field = display_field(entity);
    ...
    #[test]
    fn test_lov_item_serialization() {
        let item = LovItem {
            id: "abc-123".to_string(),
            label: "My Org".to_string(),
        };
        let json = serde_json::to_value(&item).unwrap();
        assert_eq!(json["id"], "abc-123");
        assert_eq!(json["label"], "My Org");
    }
}
