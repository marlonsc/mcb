...
/// Ordered DDL statements that build the initial schema.
///
/// Order matters: the FTS triggers reference the `observations` table, so that
/// table is created before its triggers.
...
/// FTS shadow tables dropped during rollback, before the base tables.
...
/// Base tables dropped during rollback, in reverse-dependency order.
...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        for shadow in FTS_SHADOW_TABLES {
