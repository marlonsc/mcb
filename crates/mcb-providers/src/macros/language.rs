...
//!
//! Common macros for language chunking providers.

/// Implements `LanguageProcessor` by delegating to an inner `BaseProcessor` field.
#[macro_export]
macro_rules! impl_processor_delegate {
...
/// Implements `Default` via `new()` and `LanguageProcessor` via delegation.
#[macro_export]
macro_rules! impl_delegating_language_processor {
    ($processor_ty:ty, $inner_field:ident) => {
        ...
        $crate::impl_processor_delegate!($processor_ty, $inner_field);
    };
}
