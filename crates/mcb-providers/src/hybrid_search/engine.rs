...
mod tests {
    ...
    async fn test_hybrid_search() {
        ...
        let chunks = vec![
            create_test_chunk(
                "fn authenticate_user(username: &str) -> bool { validate(username) }",
                "auth.rs",
                1,
            ),
            create_test_chunk(
                "fn process_data(data: Vec<u8>) -> Vec<u8> { compress(data) }",
                "data.rs",
                1,
            ),
        ];
        ...
        // Simulate semantic search results (from vector store)
        let semantic_results = vec![
            create_test_search_result("auth.rs", 1, 0.8),
            create_test_search_result("data.rs", 1, 0.85),
        ];

        // Hybrid search for "authenticate user"
        let results = engine
            .search("test", "authenticate user validation", semantic_results, 10)
            .await
            .unwrap();

        assert_eq!(results.len(), 2);
        // Auth chunk should rank higher due to BM25 boost for matching terms
