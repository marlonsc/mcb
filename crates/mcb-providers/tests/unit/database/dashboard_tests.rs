...
async fn insert_observation(db: &DatabaseConnection, id: &str, created_at: i64) {
    let sql = format!(
...
async fn insert_tool_call(db: &DatabaseConnection, id: &str, tool_name: &str) {
    let sql = format!(
    ...
    db.execute_unprepared(&sql).await.expect("insert tool call");
}

/// Insert test agent_sessions.
async fn insert_agent_session(db: &DatabaseConnection, id: &str, agent_type: &str) {
    let sql = format!(
...
#[tokio::test]
async fn test_observations_by_day_with_data() {
    let db = setup().await;
...
#[tokio::test]
async fn test_observations_by_day_respects_limit() {
    let db = setup().await;
...
#[tokio::test]
async fn test_tool_call_counts_empty_db() {
    let db = setup().await;
    let adapter = SeaOrmDashboardAdapter::new(db);
...
#[tokio::test]
async fn test_tool_call_counts_aggregates_by_name() {
    let db = setup().await;
    ...
    let search_count = counts
        ...
        .map(|c| c.count)
        .expect("search_code should exist");
    assert_eq!(search_count, 2, "search_code should have 2 calls");
}

#[tokio::test]
async fn test_tool_call_counts_ordered_by_count_desc() {
    let db = setup().await;
...
#[tokio::test]
async fn test_agent_session_stats_empty_db() {
    let db = setup().await;
    let adapter = SeaOrmDashboardAdapter::new(db);

    let result = adapter.get_agent_session_stats().await;
    assert!(result.is_ok(), "should succeed on empty db");
...
#[tokio::test]
async fn test_agent_session_stats_counts_sessions() {
    let db = setup().await;
...
#[tokio::test]
async fn test_agent_session_stats_counts_unique_agents() {
    let db = setup().await;
