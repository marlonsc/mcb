...
3. **Branch currently contains merged forward work**
   - `release/v0.2.1` has merges from `release/v0.2.2` and feature branches, so closure planning must separate true v0.2.1 blockers from v0.2.2+ enhancement scope.
...
### Confirmed unresolved integration gaps

- `mcp_mcb_project` not fully implemented in historical gap reports.
- Memory list/query and context unification were reported as blockers in prior validation/research artifacts.
...
## Pending closure matrix for v0.2.1 line

### A) Release-critical closure (complete before declaring v0.2.1 finalized)

1. **Re-validate branch correctness against intended release scope**
   - Confirm whether `release/v0.2.1` should include merged v0.2.2 features.
   - Output: explicit inclusion/exclusion list tied to commits.

2. **Run full quality gate for current branch state**
   - `cargo check`, targeted integration tests, startup smoke tests, and MCP contract checks.
   - Ensure no regression against v0.2.0 stability guarantees.

3. **Close or downgrade known blocker gaps that were marked hotfix-level**
   - Re-test historical GAP-1/GAP-2/GAP-4 claims on current branch head.
   - If fixed in code, update gap docs and close corresponding issues.
   - If still open, convert to explicit release blocker list with owner and ETA.

4. **Publish branch-consistent release notes**
   - Changelog entry must reflect actual commit set on `release/v0.2.1`, not stale planned state.

### B) Should remain out of v0.2.1 closure unless scope changes
...
- Treat as roadmap continuation, not v0.2.1 stabilization gate.
...
- Mixing v0.2.1 and v0.2.2+ scope causes ambiguous release claims.
...
- `31f124dc` moved line to `0.2.2-dev` (forward development continuation).
- `9fe5a8f3` merged `feat/data-model-v2` into `release/v0.2.1`.
- `b8d08230` merged `release/v0.2.2` back into `release/v0.2.1`.
...
- `mcb-vi4f`: v0.2.2 implementation history and pending closure matrix (directly related to this context stream).

### Ready queue signal (top sample)

- `mcb-gi1b`: Epic for v0.2.2 admin UI schema-driven automation.
...
### Ambiguity: v0.2.1 line includes v0.2.2 merges
...
- Risk: helper and partial expansion in v0.2.2+ may increase coupling unless rigorously tested.
