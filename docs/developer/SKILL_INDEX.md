...
workstation-local: `.gitignore` excludes `.claude/*`, so the skills are loaded by the
agent runtime rather than versioned here, and this index names them instead of linking
to repository paths that do not exist.
...
In Claude Code, invoke with `/skill:<name>` or the `Skill` tool. In other ECC
environments, use the platform-specific skill activation command.

## Maintenance

When MCB patterns change, update the corresponding skill. The skill front matter should
always cite the canonical source files (`Cargo.toml`, `Makefile`, `AGENTS.md`, etc.).
