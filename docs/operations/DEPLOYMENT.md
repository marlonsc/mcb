...
Do not use the removed `config.toml` provider format. Runtime configuration is Loco YAML
under `config/`, with MCB-specific fields under `settings:`.
...
Use the project Make verbs. They are the command SSOT for build, test, validation,
release, and Git operations.
...
directory, writes installed YAML config, updates supported MCP client configs when
present, manages the user `mcb` systemd service, and validates MCP stdio with an
initialize request.
...
The public MCP contract is documented in `docs/MCP_TOOLS.md`. It currently exposes 24
public tool names grouped into search, index, memory, session, agent, validation, VCS,
project, and entity families.
...
Kubernetes manifests in `k8s/` are declarative repository artifacts, not an imperative
deployment channel. Steady-state cluster changes must go through Git and the GitOps
controller, following `AGENTS.md`.

Open reconciliation work for the current `k8s/` manifests is tracked in beads, not in
this operations page. Use `bd show mcb-vy4k.5.12` before changing that lane.
