...
v0.4.0 introduces the**Integrated Context System** with knowledge graphs, freshness
tracking, and time-travel queries. This guide covers breaking changes, new
...
> **v0.3.0 Migration Note:** Runtime configuration now uses Loco YAML
> (`config/development.yaml`, `config/test.yaml`). The TOML snippet below is historical
> planning context.
...
Replace old tool names with new tools:
