...
get_vector_store_count() {
	local count
	count=$(find "$PROJECT_ROOT/crates/mcb-providers/src/vector_store" -maxdepth 1 -name "*.rs" ! -name "mod.rs" ! -name "filesystem.rs" ! -name "helpers.rs" 2>/dev/null | grep -c . || true)
	echo "${count:-0}" | tr -d ' '
}

get_vector_store_list() {
	find "$PROJECT_ROOT/crates/mcb-providers/src/vector_store" -maxdepth 1 -name "*.rs" ! -name "mod.rs" ! -name "filesystem.rs" ! -name "helpers.rs" -print0 2>/dev/null |
		xargs -0 -I {} basename {} .rs |
		sed 's/^milvus$/Milvus/' |
...
get_test_count() {
	...
	# Format with commas (e.g., 3088 → 3,088)
	printf "%'d" "${total}"
}

# Raw numeric test count (no comma formatting, for JSON output)
get_test_count_raw() {
	local count async_count int_count total
	count=$(find "$PROJECT_ROOT/crates" -name "*.rs" -type f -exec grep -h "^[[:space:]]*#[[:space:]]*\[test\]" {} \; 2>/dev/null | grep -c . || true)
	async_count=$(find "$PROJECT_ROOT/crates" -name "*.rs" -type f -exec grep -h "#\[tokio::test\]" {} \; 2>/dev/null | grep -c . || true)
