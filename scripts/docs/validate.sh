...
validate_adr_format() {
	...
	# Check ADR number in title (supports "ADR 001", "ADR-001", and variants)
	local has_errors=false
	local adr_num_padded
	adr_num_padded=$(printf "%03d" "$adr_num")
	local title_line
	title_line=$(grep -m1 -E '^#{1,3}[[:space:]]+ADR[-[:space:]]*[0-9]+' "$adr_file" | tr -d '\r' || true)
	if [[ -z "$title_line" ]]; then
		log_error "ADR title heading missing in $filename"
		inc_errors
		return
	fi
	...
	local required_sections=("## Context" "## Decision")
	...
	if ! grep -qE '^(##[[:space:]]+Status|\*\*Status\*\*|status:)' "$adr_file"; then
		log_error "ADR $filename missing status declaration (frontmatter, section, or metadata line)"
		has_errors=true
	fi
