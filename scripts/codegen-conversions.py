...
GENERATED_HEADER = "//! ⚠️ GENERATED by `make build WHAT=codegen ACT=conversions APPLY=Y` — DO NOT EDIT\n"
ENTITIES_DIR = (
...
def gen_from_model_field(field: str, convert: dict[str, Any]) -> str:
...
def gen_to_active_field(field: str, convert: dict[str, Any]) -> str | None:
    conv = convert.get(field)
    if not conv:
...
def needs_serde_json(convert: dict[str, Any]) -> bool:
    serde_types = {
        "json_array",
        "json_array_required",
        "json_object",
...
def gen_test_block(name: str, entity: dict[str, Any]) -> str:
...
def all_fields_ordered(entity: dict[str, Any]) -> list[str]:
...
def gen_conversion_file(name: str, entity: dict[str, Any]) -> str:
    domain_type = entity["domain"]
    entity_module = entity["entity"]
