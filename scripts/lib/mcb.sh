...
mcb_guard() {
  ...
  # Precision: only real call sites count. Doc comments, line comments, and
  # string literals (validator needles/messages) mention these tokens by
  # design and are not usage.
  hits=$(grep -rnE '\b(unwrap|expect)\(|\bpanic!|\btodo!|\bunimplemented!' $src 2>/dev/null \
      | grep -vE '//.*(unwrap|expect)|#\[cfg\(test\)\]' \
      | grep -vE ':[0-9]+: *//[!/]?' \
      | grep -vE '"[^"]*(unwrap|expect)\(|"[^"]*(panic|todo|unimplemented)!' || true)
